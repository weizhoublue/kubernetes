...
func TestGetLowerPriorityNominatedPods(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "no nominated pods",
			nominatedPods: nil,
			priority:      50,
			expectedCount: 0,
		},
		{
			name: "nominated pod without PodGroup, lower priority",
			nominatedPods: []*v1.Pod{
				st.MakePod().Name("pod1").Namespace("default").Priority(30).Obj(),
			},
			priority:      50,
			expectedCount: 1,
		},
		{
			name: "nominated pod without PodGroup, higher priority",
			nominatedPods: []*v1.Pod{
				st.MakePod().Name("pod1").Namespace("default").Priority(70).Obj(),
			},
			priority:      50,
			expectedCount: 0,
		},
		{
			name: "nominated pod with PodGroup, PodGroup priority lower than threshold",
			nominatedPods: []*v1.Pod{
				st.MakePod().Name("pod1").Namespace("default").Priority(30).PodGroupName("pg1").Obj(),
			},
			priority: 50,
			podGroups: map[string]*schedulingapi.PodGroup{
				"pg1": {
		...
		{
			name: "nominated pod with PodGroup, PodGroup priority equal to threshold - not included",
			nominatedPods: []*v1.Pod{
				st.MakePod().Name("pod1").Namespace("default").Priority(30).PodGroupName("pg1").Obj(),
			},
			priority: 50,
		...
		{
			name: "nominated pod with PodGroup, PodGroup priority higher than threshold",
			nominatedPods: []*v1.Pod{
				st.MakePod().Name("pod1").Namespace("default").Priority(70).PodGroupName("pg1").Obj(),
			},
			priority: 50,
			podGroups: map[string]*schedulingapi.PodGroup{
				"pg1": {
		...
		{
			name: "mixed pods: some with PodGroup that elevates priority above threshold",
			nominatedPods: []*v1.Pod{
				st.MakePod().Name("pod1").Namespace("default").Priority(30).PodGroupName("pg1").Obj(),
				st.MakePod().Name("pod2").Namespace("default").Priority(40).Obj(),
				st.MakePod().Name("pod3").Namespace("default").Priority(30).PodGroupName("pg2").Obj(),
			},
			priority: 50,
			podGroups: map[string]*schedulingapi.PodGroup{
				"pg1": {
		...
		{
			name: "PodGroup priority lower than pod priority is used as effective priority",
			nominatedPods: []*v1.Pod{
				st.MakePod().Name("pod1").Namespace("default").Priority(70).PodGroupName("pg1").Obj(),
			},
			priority: 50,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			podInfos := make([]fwk.PodInfo, len(tt.nominatedPods))
			for i, pod := range tt.nominatedPods {
				podInfos[i] = &framework.PodInfo{Pod: pod}
			}
			nominator := &testNominatedPodLister{podInfos: podInfos}

			result := getLowerPriorityNominatedPods(nominator, tt.priority, "node1", pgLister)
			if len(result) != tt.expectedCount {
				t.Errorf("expected %d pods, got %d", tt.expectedCount, len(result))
			}
		})
	}
}
...
type testNominatedPodLister struct {
	podInfos []fwk.PodInfo
}

func (t *testNominatedPodLister) AddNominatedPod(logger klog.Logger, pod fwk.PodInfo, nominatingInfo *fwk.NominatingInfo) {
}
func (t *testNominatedPodLister) DeleteNominatedPodIfExists(pod *v1.Pod) {}
func (t *testNominatedPodLister) UpdateNominatedPod(logger klog.Logger, oldPod *v1.Pod, newPodInfo fwk.PodInfo) {
}
func (t *testNominatedPodLister) NominatedPodsForNode(nodeName string) []fwk.PodInfo {
	return t.podInfos
}
